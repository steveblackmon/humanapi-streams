 ...
 */
public class HumanApiReadingsProvider extends SimpleHTTPGetProvider {

    private final static Logger LOGGER = LoggerFactory.getLogger(HumanApiReadingsProvider.class);
    ...
    /** resources */
    public static Collection<String> readingResources = Lists.newArrayList(
        "blood_glucose",
        "blood_oxygen",
        "blood_pressure",
        "bmi",
    ...
    public StreamsResultSet readCurrent() {
        ...
        List<URI> resourceRequests = readingResources.stream().map(resource -> {
            try {
                URIBuilder builder = new URIBuilder(uriBuilder.build());
                builder = builder.setPath(
                    Joiner.on("/").skipNulls().join(builder.getPath(), resource, configuration.getResourcePostfix())
                );
        ...
        List<ObjectNode> results = resourceRequests.parallelStream().flatMap(uri -> executeGet(uri).stream()).collect(Collectors.toList());
    ...
    public StreamsResultSet readRange(DateTime start, DateTime end) {
        ...
        for( String resource : readingResources ) {
            int offset = 0;
            try {
                uriBuilder = uriBuilder.setPath(
                    Joiner.on("/").skipNulls().join(uriBuilder.getPath(), resource, configuration.getResourcePostfix())
                );
